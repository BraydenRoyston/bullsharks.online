...
pub async fn get_activities_from_this_week(
    ...
    let start_of_week_pacific = now_pacific
        ...
        .and_hms_opt(0, 0, 0)
        .ok_or_else(|| {
            ApiError::InternalConversionError("Invalid midnight time for week start".to_string())
        })?
    ...
    let end_of_week_pacific = start_of_week_pacific
        ...
        .ok_or_else(|| {
            ApiError::InternalConversionError("Invalid end-of-day time for week end".to_string())
        })?
        + Duration::days(6);
...
pub async fn get_activities_from_this_month(
    ...
    let start_of_month_pacific = now_pacific
        ...
        .and_hms_opt(0, 0, 0)
        .ok_or_else(|| {
            ApiError::InternalConversionError("Invalid midnight time for month start".to_string())
        })?;
    ...
    .and_hms_opt(0, 0, 0)
    .ok_or_else(|| {
        ApiError::InternalConversionError("Invalid midnight time for next month".to_string())
    })?;
