...
fn test_create_hash_for_activity_consistent() {
    ...
    let composite2 = format!("{}|{}|{}|{}|{}", "John", "Doe", 10000.0, 3600, 3600);

    assert_eq!(composite1, composite2);

    // Different activities should produce different composites
    let composite3 = format!("{}|{}|{}|{}|{}", "Jane", "Doe", 10000.0, 3600, 3600);
...
fn test_create_hash_for_activity_different_athletes() {
    let composite_john = format!("{}|{}|{}|{}|{}", "John", "Doe", 10000.0, 3600, 3600);

    let composite_jane = format!("{}|{}|{}|{}|{}", "Jane", "Doe", 10000.0, 3600, 3600);
...
fn test_create_hash_for_activity_different_distances() {
    let composite_10k = format!("{}|{}|{}|{}|{}", "John", "Doe", 10000.0, 3600, 3600);

    let composite_5k = format!("{}|{}|{}|{}|{}", "John", "Doe", 5000.0, 3600, 3600);
...
fn test_create_hash_for_activity_different_times() {
    let composite_1h = format!("{}|{}|{}|{}|{}", "John", "Doe", 10000.0, 3600, 3600);

    let composite_30m = format!("{}|{}|{}|{}|{}", "John", "Doe", 10000.0, 1800, 1800);
...
fn test_valid_activity_running_only() {
    let running_activity =
        create_test_bullshark_activity("test1", "2024-01-15", "John Doe", 10.0, "Run");

    let cycling_activity =
        create_test_bullshark_activity("test2", "2024-01-15", "John Doe", 10.0, "Ride");

    let walking_activity =
        create_test_bullshark_activity("test3", "2024-01-15", "John Doe", 10.0, "Walk");
...
fn test_get_start_of_week_for_activity() {
    ...
    let wednesday_activity = create_test_bullshark_activity(
        ...
        "John Doe",
        10.0,
        "Run", // January 17, 2024 was a Wednesday
    );
    ...
    let start_of_week_wed = wednesday_date.date().and_hms_opt(0, 0, 0).unwrap()
...
fn test_convert_activity_to_bullshark_activity() {
    ...
    let _time = FixedOffset::east_opt(0)
        .unwrap()
...
fn test_activity_filtering_logic() {
    ...
    assert!(
        running_activities
            .iter()
            .all(|a| a.sport_type.as_deref() == Some("Run"))
    );
}
