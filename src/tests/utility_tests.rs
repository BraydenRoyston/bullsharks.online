...
fn test_month_boundary_calculation() {
    ...
    let next_year_jan = if dec_date.month() == 12 {
        dec_date.with_year(dec_date.year() + 1).and_then(|d| d.with_month(1))
    } else {
...
fn test_hash_input_generation() {
    ...
    let test_cases = vec![
        ("John", "Doe", 10000.0, 3600, 3600, "John|Doe|10000|3600|3600"),
        ("Jane", "Smith", 5000.0, 1800, 1900, "Jane|Smith|5000|1800|1900"),
    ...
    for (first_name, last_name, distance, moving_time, elapsed_time, expected) in test_cases {
        ...
        if distance > 0.0 { // Skip if distance is already at minimum
...
fn test_activity_validation_logic() {
    ...
    for activity in &test_activities {
        ...
        if activity.sport_type.is_none() || activity.distance.is_none() || activity.athlete_name.is_none() {
...
fn test_weekly_data_aggregation_logic() {
    ...
    let activities = vec![
        ActivityData { athlete: "John".to_string(), week: "2024-01-01".to_string(), distance_km: 10.0 },
        ActivityData { athlete: "John".to_string(), week: "2024-01-01".to_string(), distance_km: 5.0 },
        ActivityData { athlete: "Jane".to_string(), week: "2024-01-01".to_string(), distance_km: 8.0 },
        ActivityData { athlete: "John".to_string(), week: "2024-01-08".to_string(), distance_km: 12.0 },
        ActivityData { athlete: "Jane".to_string(), week: "2024-01-08".to_string(), distance_km: 7.0 },
    ];
    
    // Aggregate by athlete
    let mut athlete_totals: HashMap<String, f64> = HashMap::new();
    for activity in &activities {
        *athlete_totals.entry(activity.athlete.clone()).or_insert(0.0) += activity.distance_km;
    }
    ...
    assert_eq!(athlete_week_totals.get(&("John".to_string(), "2024-01-01".to_string())), Some(&15.0)); // 10 + 5
    assert_eq!(athlete_week_totals.get(&("Jane".to_string(), "2024-01-01".to_string())), Some(&8.0));
    assert_eq!(athlete_week_totals.get(&("John".to_string(), "2024-01-08".to_string())), Some(&12.0));
    assert_eq!(athlete_week_totals.get(&("Jane".to_string(), "2024-01-08".to_string())), Some(&7.0));
}
...
fn test_floating_point_precision_handling() {
    ...
    for (growth_percentage, should_be_no_risk) in test_cases {
        ...
        assert_eq!(is_no_risk, should_be_no_risk, 
