...
fn test_api_error_internal_conversion_error() {
    let error =
        ApiError::InternalConversionError("Failed to parse datetime from string".to_string());
...
fn test_api_error_external_api_error() {
    let error =
        ApiError::ExternalAPIError("Strava API returned 503 Service Unavailable".to_string());
...
fn test_error_categorization() {
    ...
    for error in server_errors {
        match error {
            ...
            | ApiError::StartupError(_)
            | ApiError::AuthTokenError(_)
            | ApiError::InternalConversionError(_)
...
fn test_error_chaining_scenarios() {
    ...
    let startup_error =
        ApiError::StartupError(format!("Database initialization failed: {}", db_error));
    ...
    let conversion_error = ApiError::InternalConversionError(format!(
        "Failed to parse Strava response: {}",
        api_response
    ));
...
fn test_error_context_preservation() {
    ...
    let contextual_error =
        ApiError::DatabaseError(format!("Query execution failed: {}", original_error));
...
fn test_error_logging_friendliness() {
    ...
    let errors_with_context = vec![
        ApiError::DatabaseError(
            "Query timeout: SELECT * FROM activities WHERE date > '2024-01-01'".to_string(),
        ),
        ApiError::ExternalAPIError(
            "Strava API error: 429 Rate Limit Exceeded, retry after 60s".to_string(),
        ),
        ApiError::InternalConversionError(
            "Date parsing failed: '2024-13-01T00:00:00Z' at position 5".to_string(),
        ),
    ];
