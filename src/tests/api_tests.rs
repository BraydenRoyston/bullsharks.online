...
fn test_get_activities_from_this_week_date_calculation() {
    ...
    let test_date = Los_Angeles.with_ymd_and_hms(2024, 1, 17, 15, 30, 0).unwrap();
    
    // Calculate start of week (Monday 00:00:00) in Pacific
    let days_since_monday = test_date.weekday().num_days_from_monday();
    let start_of_week_pacific = test_date
        .date_naive()
        .and_hms_opt(0, 0, 0)
        .unwrap()
        - Duration::days(days_since_monday as i64);
    let start_of_week_pacific = Los_Angeles.from_local_datetime(&start_of_week_pacific).single().unwrap();
    
    // Wednesday should be 2 days from Monday
    assert_eq!(days_since_monday, 2);
    
    // Start of week should be Monday, January 15, 2024
    assert_eq!(start_of_week_pacific.date_naive(), NaiveDate::from_ymd_opt(2024, 1, 15).unwrap());
    ...
    let end_of_week_pacific = start_of_week_pacific
        ...
        .unwrap()
        + Duration::days(6);
    let end_of_week_pacific = Los_Angeles.from_local_datetime(&end_of_week_pacific).single().unwrap();
    
    // End of week should be Sunday, January 21, 2024
    assert_eq!(end_of_week_pacific.date_naive(), NaiveDate::from_ymd_opt(2024, 1, 21).unwrap());
...
fn test_get_activities_from_this_month_date_calculation() {
    ...
    let start_of_month_pacific = test_date
        ...
        .and_hms_opt(0, 0, 0)
        .unwrap();
    let start_of_month_pacific = Los_Angeles.from_local_datetime(&start_of_month_pacific).single().unwrap();
    
    assert_eq!(start_of_month_pacific.date_naive(), NaiveDate::from_ymd_opt(2024, 1, 1).unwrap());
    
    // Calculate end of month
    let next_month = if test_date.month() == 12 {
        test_date.date_naive()
            .with_year(test_date.year() + 1)
            .and_then(|d| d.with_month(1))
    } else {
        test_date.date_naive()
            .with_month(test_date.month() + 1)
    }
    .unwrap()
    .and_hms_opt(0, 0, 0)
    .unwrap();

    let end_of_month_pacific = Los_Angeles.from_local_datetime(&next_month).single().unwrap()
        - Duration::seconds(1);
...
fn test_get_activities_from_this_month_december_edge_case() {
    ...
    let test_date = Los_Angeles.with_ymd_and_hms(2024, 12, 15, 15, 30, 0).unwrap();
...
fn test_get_activities_from_custom_window_invalid_format() {
    ...
    for invalid_format in invalid_formats {
        ...
        assert!(result.is_err(), "Expected parsing to fail for: {}", invalid_format);
    }
}
...
fn test_activities_filtering_by_time_window() {
    ...
    let window_start = NaiveDate::from_ymd_opt(2024, 1, 14).unwrap()
        .and_hms_opt(0, 0, 0).unwrap();
    let window_end = NaiveDate::from_ymd_opt(2024, 1, 22).unwrap()
        .and_hms_opt(23, 59, 59).unwrap();
    
    let window_start_tz = FixedOffset::east_opt(0).unwrap().from_local_datetime(&window_start).unwrap();
    let window_end_tz = FixedOffset::east_opt(0).unwrap().from_local_datetime(&window_end).unwrap();
    ...
    assert!(filtered_activities.iter().all(|a| a.athlete_name == Some("John Doe".to_string())));
}
